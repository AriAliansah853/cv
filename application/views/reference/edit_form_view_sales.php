...
<body class="hold-transition sidebar-mini">
			        ...
			        <div class="row">
						...
						<div class="card card-danger">
				              	...
				              	<!-- <?php $data = $this->db->query("SELECT * FROM userlogin where login_id = '".$inser_user."'");
				              		$User = $data->row_array();
				              	?> -->
				              	<form role="form" action="<?php echo base_url('reference/update_sales/'.$id);?>" method="post" >
					                <div class="card-body">
					                  	<div class="form-group">
						                    <label>Nama Sales</label>
						                    <input type="text" name="sales" class="form-control" value="<?php echo $nama_sales; ?>">
					                  	</div>
					                  	<div class="form-group">
						                    <label>Visible</label>
	...
	<script>
	var urlAdd="<?php echo base_url('reference/add_sales');?>";
	var link="<?php echo base_url('reference/edit_sales/').''.$rsales_id;?>";
	  	$(function () {
	    	$("#example1").DataTable();
	    	$('#example2').DataTable({
	      		"paging": true,
		...
		var cekError=<?php echo $errorData?>;
		 if(cekError == 0){
            swal({
                title: 'Sales Sudah Ada',
                icon: "warning",
            })
            .then((Ok) => {
                $('#btnSubmit').text('Tambah Sales Gagal'); 
